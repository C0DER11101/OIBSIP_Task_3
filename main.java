	...
	public static void main(String args[]) {
		...
		if(moneyWithdraw.exists()) {
			...
			try {
				...
				createColumn.write(String.format("%s %30s %30s %30s", "Date and time", "Balance before", "Amount withdrawn", "Balance now\n"));
				createColumn.close();
			}
		...
		if(moneyDeposit.exists()) {
			...
			try {
				...
				try {
					...
					createColumn.write(String.format("%s %30s %30s %30s", "Date and time", "Balance before", "Amount deposited", "Balance now\n"));
					createColumn.close();
				}
